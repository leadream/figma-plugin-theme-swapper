...
function walk (sel, originalTheme, targetTheme) {
  ...
  if (sel.children && sel.children.length>0) {
    // 如果有子图层，对每个子图层再递归执行一遍 walk
    sel.children.map(node => {
      walk(node, originalTheme, targetTheme)
    })
  }
}
...
export function swapTheme (targetTheme) {
  ...
  if (sels.length===0) {
    ...
    // 循环处理每一个所选元素
    sels.map(sel => {
      walk(sel, originalTheme, targetTheme)
    })
  }
}
